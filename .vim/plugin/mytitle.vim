...
func SetCPPTitle()
    ...
    call append(i+6, "  */")
    call append(i+7, "")
    call append(i+8, "#include <iostream>")
    call append(i+9, "#include <unistd.h>")
    call append(i+10, "")
    call append(i+11, "/*--------------------------------extern--------------------------------------------*/")
    call append(i+12, "")
    call append(i+13, "/*--------------------------------globals-------------------------------------------*/")
...
func SetMKTitle()
    ...
    call append(i+11, "STRIP ?= strip")
    call append(i+12, "CFLAGS = -g -Wall -I $(DIR_INC)")
    call append(i+13, "")
    call append(i+14, "${BIN_TARGET}: ${OBJ}")
    call append(i+15, "	$(warning Building $(BIN_TARGET) ...)")
    call append(i+16, "	@$(CC) $(OBJ) -o $@")
    call append(i+17, "	@$(STRIP) $@")
    call append(i+18, "")
    call append(i+19, "${DIR_OBJ}/%.o: ${DIR_SRC}/%.c")
    call append(i+20, "	$(warning Comping $< ...)")
    call append(i+21, "	@$(CC) $(CFLAGS) -c $< -o $@")
    call append(i+22, "")
    call append(i+23, "# Prevent naming conflicts such as the existence of a file named clean.")
    call append(i+24, ".PHONY: clean")
    call append(i+25, "")
    call append(i+26, "clean:")
	call append(i+27, "	$(warning Clean $(OBJ), $(BIN_TARGET) ...)")
    call append(i+28, "	@find ${DIR_OBJ} -name *.o -exec rm -rf {} \\;")
    call append(i+29, "	@rm -rf $(BIN_TARGET)")
    call cursor(i+9, 13)
endfunc
" VIM-FILE TITILE {{{1
func SetVimTitle()
